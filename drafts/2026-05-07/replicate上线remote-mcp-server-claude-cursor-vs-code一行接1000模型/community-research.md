/last30days · researching: Replicate MCP server
...
[Planner]   sq1 label=primary search="replicate mcp server" sources=[hackernews,reddit]
✓ Research complete (1.4s) - Reddit: 1 thread, HN: 0 stories, Github: 0 results
...
# last30days v3.0.0: Replicate MCP server
...
⚠️  This run was called BARE on a named-entity topic ("Replicate MCP server"):
...
### 1. llama-server / web gui / C++ mcp server : is it possible to inject context (for skills or text flavour)? (score 0, 1 item, sources: Reddit)
1. [reddit] llama-server / web gui / C++ mcp server : is it possible to inject context (for skills or text flavour)?
   - 2026-04-19 | r/LocalLLaMA | [2pts, 10cmt] | score:0
   - URL: https://www.reddit.com/r/LocalLLaMA/comments/1spqizd/llamaserver_web_gui_c_mcp_server_is_it_possible/
...
Now I am trying to figure out how to feed some context to the llama-server/web...

## Stats

- Total evidence: 1 item across 1 source
- Top voices: r/LocalLLaMA
- Reddit: 1 item | 2pts, 10cmt | communities: r/LocalLLaMA
...
then rerun `/last30days Replicate MCP server`. The skill will resolve handles
...
├─ 🟠 Reddit: 1 thread │ 2 upvotes │ 10 comments
