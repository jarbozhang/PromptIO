# Simon Willison: Claude Code + HTML 的不合理有效性 一份 prompt 出可发布原型

我最近又被 Simon Willison 戳了一下。

他 5 月 8 日写了一篇短文，标题叫 "The Unreasonable Effectiveness of HTML"。我一开始没太在意，以为又是某种"HTML 复兴"的怀旧叙事。点进去看完，发现是一篇讲 vibe coding 的方法论文章，而且讲到了一个我自己跑了三个月 Claude Code 都没意识到的点。
...
整个 HTML 文件 35KB，浏览器打开秒级渲染，没有任何外部依赖。我把它扔到我们团队的内网静态资源服务器上，发了个链接给同事。
...
国内的 Tongyi Code、通义灵码、Trae 我都用过，它们和 Cursor 走的是同一条路，IDE 里补全、项目里生成。这是一条扎实的路。但 Simon 的路是另一条，他不进 IDE，他直接在 Claude Code 终端里要一个 HTML 文件，做完就用，用完就扔，不需要 git 不需要 npm install 不需要部署。
...
代码审查的辅助报告。Thariq Shihipar 在 Simon 原文里给了另一个 prompt 范例，叫 Claude 帮他审 PR 时，让它生成一个 HTML artifact，把 diff 渲染出来，旁注用颜色按严重程度标记。我试了一下，比 GitHub 自带的 PR review UI 体验好。
...
教学材料。我给我家小孩讲编程概念的时候发现这个模式特别好用，让 Claude 生成一个带可视化和小练习的 HTML 页，比 Markdown 教程效果好太多。
...
第一是有人觉得 HTML artifact 取代不了真正的工程项目，这话对，但是 Simon 也没说要取代，他说的是"原型"和"解释"这两个场景。
...
- Simon 引用的 Thariq Shihipar 的 PR review prompt 示例在原文中
...
相关主题:: [[ai-coding-tools|AI 编程工具]] | [[vibe-coding|Vibe Coding]] | [[ai-methodology|AI 方法论]]
